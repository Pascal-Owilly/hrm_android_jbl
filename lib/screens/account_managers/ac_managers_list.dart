...
void main() {
  runApp(MaterialApp(
    ...
    initialRoute: '/',
    routes: {
      '/': (context) => const AccountManagersListScreen(),
    },
  ));
}

class AccountManagersListScreen extends StatefulWidget {
  const AccountManagersListScreen({super.key});
...
class _AccountManagersListScreenState extends State<AccountManagersListScreen> {
  ...
  Future<void> fetchUsers() async {
    try {
      var url = Uri.parse('${BASE_URL}/api/account-managers/');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('All HRs | JBL'), // Update screen title
      ),
